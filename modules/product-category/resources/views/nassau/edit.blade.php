...
@section('content')
<div class="row pl-3">
    <form action="{{ route('product-categories.update',$category->id) }}" method="post" class="col-md-6 col-lg7 pl-0">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="exampleInputCategoryName">Category Name</label>
            <input type="text" name="name" class="form-control" id="exampleInputCategoryName" value="{{ $category->name }}" />
        </div>
        <div class="form-group">
